...
export default function Cut() {
  ...
  return (
    <section id="cut" ref={secRef} className="relative h-screen overflow-hidden border-t border-line bg-bg">
      <div className="h-full flex flex-col pt-20 md:pt-24">
        <div className="max-w-[1300px] w-full mx-auto px-5 md:px-10 flex items-end justify-between gap-6">
          <div>
            <p className="font-mono text-[9px] tracking-[0.35em] text-mut">SOURCING // THE SMASH INGREDIENTS</p>
        ...
        <div className="flex-1 flex items-center">
          <div ref={trackRef} className="flex gap-5 pl-5 md:pl-10 pr-10 w-max will-change-transform">
            {CUT_CARDS.map((c) => (
              <article
                key={c.n}
              ...
              >
                ...
                <span className="absolute top-4 right-4 font-disp text-4xl md:text-5xl outline-gold opacity-80">{c.n}</span>
                <div className="absolute left-4 right-4 bottom-4">
                  <h3 className="font-disp font-semibold tracking-[0.05em] text-lg md:text-xl text-ink">{c.name}</h3>
        ...
        <div className="pb-6 px-5 md:px-10">
          <div className="h-px bg-line relative overflow-hidden">
            <div ref={progRef} className="absolute inset-0 bg-gold origin-left" style={{ transform: 'scaleX(0)' }} />
          </div>
          <p className="mt-3 text-center font-mono text-[9px] tracking-[0.35em] text-ink/60">SCROLL TO CONTINUE →</p>
        </div>
      </div>
    </section>
  );
