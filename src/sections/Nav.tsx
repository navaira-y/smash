    ...
    <>      <div className="fixed top-0 left-0 right-0 z-[70] h-[2px] bg-line/60">
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-[60] transition-colors duration-500 border-b ${
          scrolled ? 'bg-bg/85 backdrop-blur-md border-line' : 'bg-gradient-to-b from-black/70 to-transparent border-transparent'
        }`}
      >
        <div className="flex items-center justify-between h-[52px] px-4 md:px-8">
          <button onClick={() => go('top')} className="text-left leading-none group">
          ...
          >
            ...
            <span className="flex flex-col gap-[3px]">
              <span className={`block h-px bg-current transition-transform ${open ? 'translate-y-[2px] rotate-45' : ''} w-4`} />
              <span className={`block h-px bg-current transition-transform ${open ? '-translate-y-[2px] -rotate-45' : ''} w-4`} />
            </span>
          </button>
        </div>
      ...
      >
        ...
        <div className="h-full flex flex-col items-center justify-center gap-1 px-6">
          <p className="font-mono text-[9px] tracking-[0.4em] text-mut mb-6">SMASH // INDEX</p>
          {NAV_LINKS.map((l, i) => (
            <button
              key={l.id}
              onClick={() => go(l.id)}
            ...
            >
              <span className="font-mono text-[10px] text-gold align-top mr-3">{l.label.split('.')[0]}</span>
              {l.label.split('. ')[1]}
            </button>
          ))}
          <button
