...
export default function Build({
  ...
  const toggleTop = (i: number) => {
    ...
    onChange({ ...state, tops: has ? state.tops.filter((t) => t !== i) : [...state.tops, i] });
  };

  return (
    <section id="build" className="relative py-24 md:py-32 border-t border-line bg-bg2">
          ...
          <div className="lg:col-span-5 space-y-10">
            <Reveal>
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => onChange({ burger: state.burger, patty: 0, tops: [] })}
              ...
              <div className="grid grid-cols-3 gap-3">
                {PATTIES.map((p, i) => (
                  ...
                  >
                    <span className="font-mono text-[9px] text-mut">{p.size}</span>
                    <span className="block mt-6 font-mono text-[9px] tracking-[0.15em] text-ink">{p.name}</span>
            ...
            <Reveal>
              <p className="font-mono text-[10px] tracking-[0.3em] text-mut mb-4">STEP 2 · ADD TOPPINGS</p>
            ...
            <div className="border border-line bg-panel h-full flex flex-col">
              <div data-preview-box className="relative h-[300px] md:h-[380px] overflow-hidden bg-black">
                ...
                <div className="absolute left-5 bottom-4">
                  <span className="font-mono text-[9px] tracking-[0.3em] text-mut">SELECTED BUILD</span>
                  <h3 className="font-disp font-semibold text-2xl tracking-[0.05em] text-ink glow-soft">{burger.name}</h3>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {burger.tags.map((t) => (
                      <span key={t} className="font-mono text-[8px] tracking-[0.18em] text-gold/90 border border-gold/30 bg-black/50 px-2 py-0.5">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="p-5 md:p-6 font-mono text-[10px] tracking-[0.18em]">
                ...
                {state.patty > 0 && (
                  <div className="flex justify-between py-2 border-b border-line/70 text-mut">
                    ...
                    <span className="text-ink">+${PATTIES[state.patty].extra}</span>
                  </div>
                )}
                {state.tops.map((i) => (
                  <div
                ...
                <div className="flex items-center justify-between pt-4">
                  ...
                  <span ref={totalRef} className="font-digi text-3xl glow-gold">$0</span>
                </div>
                <button
