  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        if (countRef.current) countRef.current.textContent = String(Math.round(p * 230));
        if (fireRef.current) fireRef.current.style.opacity = String(Math.min(1, p * 1.5));
  ...
  return (
    <section id="sear" ref={secRef} className="relative h-[300vh]">
      <div className="sticky top-0 h-screen overflow-hidden bg-black">
        ...
        />
        <div ref={warmRef} className="absolute inset-0 bg-ember opacity-0 mix-blend-overlay" />
        <img
          ref={fireRef}
          src="images/hero-02.jpg"
          alt=""
          className="absolute inset-x-0 bottom-0 w-full h-[60%] object-cover opacity-0 mix-blend-screen"
          style={{ maskImage: 'linear-gradient(to top, black 25%, transparent 95%)', WebkitMaskImage: 'linear-gradient(to top, black 25%, transparent 95%)' }}
        />
        ...
        <div className="relative h-full max-w-[1300px] mx-auto px-5 md:px-10 flex flex-col">          <div className="pt-24 md:pt-28">
          ...
          <div className="flex-1 flex items-center justify-between gap-8">            <div className="max-w-xl">
              ...
              <div className="text-right">
                ...
                <p className="mt-2 font-mono text-[9px] tracking-[0.3em] text-mut">GRIDDLE TEMP · RISING</p>
              </div>
              <div className="hidden md:flex flex-col items-center gap-2 pb-2">
                ...
                <div className="relative h-[38vh] w-[3px] bg-line overflow-hidden">
                  <div ref={barRef} className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-ember to-gold" style={{ height: '0%' }} />
                </div>
          ...
          <div className="pb-8 text-center">
            <span className="font-mono text-[9px] tracking-[0.35em] text-ink/70 flicker">SCROLL TO RISE HEAT</span>
          </div>
        </div>
      </div>
    </section>
