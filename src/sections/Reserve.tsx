...
export default function Reserve() {
  ...
  return (
    <section id="reserve" className="relative py-28 md:py-36 border-t border-line bg-bg overflow-hidden">
      <img src="images/booking.jpg" alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.14]" />
          ...
          <div className="border border-line bg-panel/90 backdrop-blur p-6 md:p-10 grid md:grid-cols-2 gap-10">
            {done ? (
              <div className="md:col-span-2 py-16 text-center">
                <p className="font-mono text-[10px] tracking-[0.35em] text-gold">// RESERVATION LOGGED</p>
                ...
                <p className="mt-4 font-mono text-[11px] tracking-[0.2em] text-mut">
                  {MONTHS[view.getMonth()]} {day} · {time} · {party} GUEST{party > 1 ? 'S' : ''}
              ...
              <>                <div>
                  ...
                  <div className="border border-line bg-bg2 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <button onClick={() => { setOff((o) => o - 1); setDay(null); }} className="text-mut hover:text-gold px-2 transition-colors">‹</button>
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center">
                      {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                        <span key={i} className="font-mono text-[8px] text-dim py-1">{d}</span>
                      ))}
                  ...
                  <p className="font-mono text-[10px] tracking-[0.3em] text-mut mt-6 mb-3">03 · PARTY SIZE</p>
                  <div className="flex items-center justify-between border border-line bg-bg2 px-4 py-3">
                    <button onClick={() => setParty((p) => Math.max(1, p - 1))} className="text-mut hover:text-gold text-lg px-2 transition-colors">−</button>
                    ...
                    {LUNCH.map((t) => (
                      <button
                        ...
                        className={`h-9 font-mono text-[10px] border transition-all ${
                          time === t ? 'border-gold bg-gold text-bg' : 'border-line bg-bg2 text-mut hover:border-gold/50 hover:text-ink'
                        }`}
                      >
                        {t}
                      </button>
                  ...
                  <p className="font-mono text-[8px] tracking-[0.3em] text-dim mt-4 mb-2">DINNER SERVICE</p>
                  <div className="grid grid-cols-4 gap-2">
                    {DINNER.map((t) => (
                      <button
                        key={t}
                  ...
                  <p className="font-mono text-[10px] tracking-[0.3em] text-mut mt-6 mb-3">04 · YOUR DETAILS</p>
                  <input
                    value={name}
